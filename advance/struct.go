...
func Structs() {
	...
	type book struct { //each field must be unique inside a struct
		title  string
		author string
		year   int
	}
	...
	fmt.Println(lastBook) // {The Divine Comedy Dante Aligheri 1320}
	...
	fmt.Println(bestBook) // {Animal Farm George Orwell 1945}
	...
	inside1 := sis{name: "lastbook", test: lastBook}

	fmt.Println(inside1)

	fmt.Println(strings.Repeat("--", 20))

	lastBook.title = "new title" // this will not be reflected in inside1
	...
	// accessing a field
	fmt.Printf("Employee's salary: %d\n", john.salary) // Employee's salary: 3000
