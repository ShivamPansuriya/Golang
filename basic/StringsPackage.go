...
func main() {
	...
	myStr = strings.Replace("10.20.40.239", ".", ":", -1)

	p(myStr) // -> 192:168:0:1

	// ReplaceAll() returns a copy of the string s with all non-overlapping instances of old replaced by new.
	myStr = strings.ReplaceAll("10.20.40.239", ".", ":")
	...
	p(utf8.RuneCountInString(s3))

	bt := []rune("你好 hello there")
	...
	for i := 0; i < len(s4); {
		runeValue, width := utf8.DecodeRuneInString(s4[i:])
		fmt.Printf("%#U starts at byte position %d\n", runeValue, i)
		i += width
	}
}
